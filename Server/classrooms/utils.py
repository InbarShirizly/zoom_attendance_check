...
def create_chat_df(chat_stream):

    chat_file = chat_stream.read().decode("utf-8").split("\n")
    ...
    chat_df = pd.DataFrame(chat_content, columns=["time", "users", "chat"])
    chat_df['chat'] = chat_df['chat'].str[:-1].astype(str)
